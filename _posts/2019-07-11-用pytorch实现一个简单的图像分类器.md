...
&emsp;训练过程主要见函数`train_model()`，实际上参考了[pytorch官方教程](https://pytorch.org/tutorials/beginner/transfer_learning_tutorial.html)。在其上增加了利用`tensorboardX`记录训练过程中`loss`和`accuracy`的变化情况以及可视化网络结果的代码，训练完后可以利用`tensorboard --logdir log`查看训练过程中`loss`和`accuracy`的变化情况以及可视化网络结构。
