...
&emsp;训练过程主要见函数`train_model()`，实际上参考了[pytorch官方教程](https://pytorch.org/tutorials/beginner/transfer_learning_tutorial.html)。在其上增加了利用`tensorboardX`记录训练过程中`loss`和`accuracy`的变化情况的代码，训练完后可以利用`tensorboard --logdir log`查看训练过程中`loss`和`accuracy`的变化情况。下图所示是在我自己的数据上训练了150个`epoch`的结果：
