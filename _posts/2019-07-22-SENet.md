...
&emsp;SE块是一个计算单元，可以建立在将输入$X\in\mathbb{R}^{H^{'}\times W^{'}\times C^{'}}$映射到特征图$U\in\mathbb{R}^{H\times W\times C}$的变换$F_{tr}$上。在接下来的符号中，我们把$F_{tr}$看作卷积运算符，并使用$V=[v_{1},v_{2},\dot,v_{C}]$表示所学习的滤波核集，其中$v_{c}$表示第c个滤波器的参数。然后我们可以写出输出$U=[u_{1},u_{2},\dot,u_{C}]$，其中
