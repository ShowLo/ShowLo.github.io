...
&emsp;SE构建块的结构如图1所示。对于任意给定的变换$F_{tr}$（例如卷积）将输入$X$映射到特征图$U$，其中$U\in \mathbb{R}^{H\times W\times C}$，我们可以构造一个对应的SE块来进行特征重校准。特征$U$首先通过挤压操作传递，该操作通过在空间维度($H\times W$)上聚合特征图来生成通道描述符。该描述符的功能是生成一个嵌入通道特征响应的全局分布，允许来自网络的全局感受域的信息被它的所有层使用。挤压之后是激励操作，该操作采用简单的自选门机制的形式，以嵌入作为输入，并产生每个通道调制权值的集合。这些权值被应用到特征图$U$中，生成SE块的输出，可以直接输入到网络的后续层中。
...
&emsp;通过简单地叠加一组SE块，就可以构造一个SE网络(SENet)。此外，这些SE块还可以在网络体系结构的一大片深度范围内作为原始块的替代品（见6.4节）。虽然构建块的模板是通用的，但是在整个网络中，它在不同深度扮演的角色是不同的。在较前的层中，它以一种与类别无关的方式激发信息特性，从而增强了共享的底层表征。在较后的层中，SE块变得越来越专门化，并以高度特定于类别的方式响应不同的输入（第7.2节）。 因此，SE块执行的特征重校准的好处可以通过网络累积。
