...
&emsp;接下来，我们在网络的最后阶段观察到一些不同的现象。SE\_5\_2表现出一种有趣的趋势，即饱和状态，其中大部分激活都接近于1。在所有激活取值1的点处，一个SE块退化为恒等算子。在SE\_5\_3网络的末端（紧接着是分类器之前的全局池），在不同的类上出现了类似的模式，规模变化不大（可以由分类器调整）。
