...
$$
\tidle{\mathbf{x}}_{c}=\mathbf{F}_{scale}(\mathbf{u}_{c},s_{c})=s_{c}\mathbf{u}_{c}
$$

&emsp;其中$\tidle{X}=[\tidle{x}_{1},\tidle{x}_{2},\ldots,\tidle{x}_{C}]$，$\mathbf{F}_{scale}(\mathbf{u}_{c}, s_{c})为标量$s_{c}$与特征图$\mathbf{u}_{c}\in\mathbb{R}^{H\times W}$之间的通道相乘。
