...
&emsp;较大的尺寸通常意味着更多的参数，这使得扩大后的网络更容易发生过拟合，特别是当训练集中标记的样本数量有限时。这是一个主要的瓶颈，因为强标记数据集的获取既费力又昂贵，常常需要专业的人工评分员来区分各种细粒度的可视类别，比如ImageNet中的那些（即使是在1000类ILSVRC子集中），如图1所示。
