...
&emsp;1. 从平台感知NAS发现的种子网络体系结构开始。

&emsp;2. 对于每个步骤:

&emsp;&emsp;(a)生成一组新proposal。每一个proposal都代表了一个架构的修改，与前一步相比，该架构至少能降低延迟$\delta$。

&emsp;&emsp;(b)对于每一个proposal，我们使用前一步骤的预先训练的模型，并填充新提出的架构，适当地截断和随机初始化缺失的权重。对每个proposal进行T个步骤的微调，以获得对精度的粗略估计。

&emsp;&emsp;(c)根据某种标准选择最佳proposal。

&emsp;3. 重复前面的步骤，直到达到目标延迟。
