...
&emsp;之前介绍MobileNet的时候说过深度可分离卷积，不过说起来最早利用深度可分离卷积的还得追溯到这篇Xception的论文，该文章发表在了CVPR 2017上，原文可见[Xception: Deep Learning with Depthwise Separable Convolutions](https://arxiv.org/abs/1610.02357)。
...
&emsp;卷积层尝试在具有2个空间维度（宽、高）和通道维度的三维空间中学习滤波器；因此，单个卷积核的任务是同时映射跨通道关联和空间关联。

&emsp;Inception模块背后的想法是通过显式地将其分解为一系列独立地查看跨通道关联和空间关联的操作，从而使这个过程更加简单和高效。
