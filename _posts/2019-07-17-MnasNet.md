...
$$
\begin{itemize}
\item 卷积运算操作ConvOp:常规卷积 (conv)、深度卷积(dconv)、移动倒瓶颈卷积[29]
\item 卷积核大小KernelSize: 3x3, 5x5
\item 挤压-激励比SERatio: 0,0.25
\item skip操作 SkipOp:池化，一致残差（identity residual），或没有skip
\item 输出滤波器尺寸F_{i}
\item 每个块的层数N_{i}
\end{itemize}
$$